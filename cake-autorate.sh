...
start_pinger()
{
	...
	case ${pinger_binary} in
		...
		fping)
			...
			( true | ${ping_prefix_string} fping ${ping_extra_args} --timestamp --loop --period "${reflector_ping_interval_ms}" --interval "${ping_response_interval_ms}" --timeout 10000 "${reflectors[@]:0:"${no_pingers}"}" 2> /dev/null >&"${pinger_fds[pinger]}" ) &
		;;
		ping)
			exec {pinger_fds[pinger]}<> <(:)
			sleep_until_next_pinger_time_slot ${pinger}
			( true | ${ping_prefix_string} ping ${ping_extra_args} -D -i "${reflector_ping_interval_s}" "${reflectors[pinger]}" 2> /dev/null >&"${pinger_fds[pinger]}" ) &
		;;
	esac
