...
printf "Running CPU benchmark for test period of: ${test_period_s}s.\n...\n"
...
cstart=$(cat /sys/fs/cgroup/services/cake-autorate/cpu.stat | awk 'NR==2,NR==3{sum+=$2};END{print sum;}')

sleep ${test_period_s}

tstop=${EPOCHREALTIME/.}
cstop=$(cat /sys/fs/cgroup/services/cake-autorate/cpu.stat | awk 'NR==2,NR==3{sum+=$2};END{print sum;}')
